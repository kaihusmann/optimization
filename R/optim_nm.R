...
optim_nm <- function(fun, k = 0, start, maximum = FALSE, trace = FALSE, alpha = 1, beta = 2, 
  ...
  if (maximum == TRUE) {     
     ...
     M <- apply(kparam, 2, mean) # calculating the mean of the k best vertices
     End <- sqrt(sum((((simplex[, 1] - fun(M)) ^ 2)) / (k + 1))) # computing some kind of 'standard error' of the simplex
     counter <- 0     
     ...
     while (tol < End) { # loop is executed until a certain auccuaricy or maximum number of iterations are reached.
       ...
       End <- sqrt(sum((simplex[, 1] - fun(M)) ^ 2) / (k + 1)) # determine the exit out of the loop if a certain accuracy is reached
  ...
  # Minimizing Function -----------------------------------------------------------------------------------
     ...
     param <- simplex[, -1] # extracting the parameters from simplex
     kparam <- param[-(k + 1), ] # ectracting the k best vertices from simplex
     M <- apply(kparam, 2, mean)   # calculating the mean of the k best points
     End <- sqrt(sum((((simplex[, 1] - fun(M)) ^  2)) / (k + 1))) # computing some kind of 'standard error' of the simplex
     counter <- 0
